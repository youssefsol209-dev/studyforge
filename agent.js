 ...
 *
 * Always parses as single-column (layout='single').
 */
async function generateMCQs(file, layout = 'single') {
  const tmpDir  = os.tmpdir();
  const ext     = path.extname(file.originalname) || '.pdf';
  const inFile  = path.join(tmpDir, `studyforge_in_${Date.now()}${ext}`);
  ...
  // Escape Windows backslashes for use inside Python raw strings
  const pyInFile  = inFile.replace(/\\/g, '\\\\');
  const pyOutJson = outJson.replace(/\\/g, '\\\\');
  const pyDir     = __dirname.replace(/\\/g, '\\\\');
  const pyLayout  = layout === 'two-column' ? 'two-column' : 'single';
  ...
  const pySnippet = [
    ...
    '',
    `questions = parse_file(r'${pyInFile}', layout='${pyLayout}')`,
    '',
    '# Debug',
    'if questions:',
    ...
    '    answers = [i for i, l in enumerate(LETTERS[:len(choices_list)]) if l in correct_letters]',
    "    output.append({'question': q['stem'], 'choices': choices_list, 'answers': answers})",
    '',
    `with open(r'${pyOutJson}', 'w', encoding='utf-8') as f:`,
